...
public class CardService extends HostApduService {
	...
	private static final String SAMPLE_LOYALTY_CARD_AID = "F999999999";
	...
	/**
	 * Called if the connection to the NFC card is lost, in order to let the application know the cause for the disconnection (either a lost link, or another AID being selected by the reader).
	 * 
	 * @param reason
	...
	public byte[] processCommandApdu(byte[] commandApdu, Bundle extras) {
		...
		if (BaseActivity.getTopActivity() instanceof AccountsInfoActivity){
			if (AccountsInfoActivity.isShow){
				showTextStr = AccountsInfoActivity.code;
			} else {
				return UNKNOWN_CMD_SW;
			}
		}
		...
		if (Arrays.equals(SELECT_APDU, commandApdu)) {
			byte[] accountBytes = showTextStr.getBytes();
			Log.e(TAG, "Sending account number: " + showTextStr);
